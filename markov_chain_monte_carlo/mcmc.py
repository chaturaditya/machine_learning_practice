...
def sample_beta_0(y, x, beta_1, beta_2, tau, mu_0, tau_0):
    ...
    for i in range(N):
        ans += beta_1*x[i]+beta_2*x[i]**2-y[i]
    mean = (mu_0*tau_0-tau*ans)/precision
...
def sample_beta_1(y, x, beta_0, beta_2, tau, mu_1, tau_1):
    ...
    for i in range(N):
        ans1 += x[i]**2
        ans2 += x[i]*(beta_0+beta_2*x[i]**2-y[i])
    
    precision = tau*ans1 + tau_1
        
    mean = (mu_1*tau_1-tau*ans2)/precision
...
def sample_beta_2(y, x, beta_0, beta_1, tau, mu_2, tau_2):
    ...
    for i in range(N):
        ans1 += x[i]**4
        ans2 += x[i]**2*(beta_0+beta_1*x[i]-y[i])
    
    precision = tau*ans1 + tau_2
        
    mean = (mu_2*tau_2-tau*ans2)/precision
    return np.random.normal(mean, 1 / np.sqrt(precision))
...
def sample_tau(y, x, beta_0, beta_1, beta_2, alpha, beta):
    ...
    ans = 0
    for i in range(N):
        ans += (y[i]-(beta_0+beta_1*x[i]+beta_2*x[i]**2))**2
    beta_new = beta+0.5*ans  
